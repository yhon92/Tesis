...
$sentencia = "SELECT CONCAT(a.nombres,' ',a.apellidos) AS nombre, a.cedula_alumno, date_format(a.fecha_nacimiento,'%d-%m-%Y') AS fecha, a.sexo, (i.nombre) AS instrumento,  date_format(a.fecha_nacimiento,'%Y') AS ano FROM alumno a, instrumento i, alumno_instrumento ai, catedra c, alumno_catedra ac WHERE a.cedula_alumno=ac.cedula_alumno AND c.cod_catedra=ac.cod_catedra AND a.cedula_alumno=ai.cedula_alumno AND ai.cod_instru=i.cod_instru AND c.cod_catedra='$cod' ORDER BY i.orden, ano DESC";

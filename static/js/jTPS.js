 ...
 (function($) {
	...
	$.fn.jTPS = function ( opt ) {
		...
		$(this).data('tableSettings', $.extend({
			perPages:			[5, 10, 15, 20, 50, 'ALL'],				// the "show per page" selection
			...
			function () {
				...
				$(this).bind('click', 
					function () { 
						...
						// $('>.nav .status', pT).html( 'Mostrando ' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' of ' + rowCount + '' );
						$('>.nav .status', pT).html( 'Mostrando: ' + ( cPos + 1 ) + ' a ' + ( cPos + ePos ) + ' de ' + rowCount + '' );
						clearSelection();
						// callback function after pagination renderd
						$(pT).data('tableSettings').clickCallback();
					}
		...
		$('>.nav .status', this).html( 'Mostrando: ' + ( cPos + 1 ) + ' a ' + ( cPos + ePos ) + ' de ' + rowCount );
